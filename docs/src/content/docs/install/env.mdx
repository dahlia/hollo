...
### `DRIVE_DISK` <Badge text="Required" />
...
<Aside type="caution">
  Switching storage drivers in production requires careful migration of existing assets.
  If you're migrating from S3 to filesystem storage:
  1. Back up your existing S3 data
  2. Set up the new filesystem storage
  3. Migrate existing assets to the new location
  4. Update the configuration
</Aside>

### `ASSET_URL_BASE` <Badge text="Required" />
...
#### `S3_BUCKET` <Badge text="Required with s3 driver" />

The bucket name of the S3-compatible object storage, e.g., `hollo`.

#### `S3_ENDPOINT_URL` <Badge text="Required with s3 driver" />
...
#### `AWS_ACCESS_KEY_ID` <Badge text="Required with s3 driver" />

The access key for S3-compatible object storage.

#### `AWS_SECRET_ACCESS_KEY` <Badge text="Required with s3 driver" />
